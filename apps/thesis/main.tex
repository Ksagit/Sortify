...
\begin{enumerate}
    \item \textbf{Faza przygotowawcza (Warm-up)}: Przed startem właściwego pomiaru, dany algorytm był uruchamiany trzykrotnie na małym zbiorze danych. Miało to na celu wymuszenie na silniku V8 kompilacji JIT (\textit{Just-In-Time}) najczęściej używanych ścieżek kodu \cite{V8_Engine_Docs}, tak aby właściwy pomiar dotyczył zoptymalizowanej wersji binarnej, a nie interpretowanego kodu JavaScript.
    \item \textbf{Inicjalizacja środowiska}: Wymuszenie operacji \textit{Garbage Collection} poprzez interfejs programistyczny DevTools w celu oczyszczenia sterty \cite{Chrome_Runtime_Performance}. Następnie następowało 5-sekundowe oczekiwanie na ustabilizowanie się obciążenia procesora do poziomu spoczynkowego.
    \item \textbf{Rejestracja właściwa}: Uruchomienie profilera nagrywającego ścieżkę wydajności jednocześnie z wyzwoleniem algorytmu sortowania \cite{Chrome_Runtime_Performance}. Rejestracja trwała dokładnie do momentu osiągnięcia stanu końcowego (posortowana tablica).
    \item \textbf{Eksport i czyszczenie}: Zapisanie surowych danych profilowania do formatu JSON oraz wykonanie zrzutu pamięci sterty \cite{Chrome_Runtime_Performance}.
\end{enumerate}
...
Rozdział ten odnosi się do wersji aplikacji ujednoliconych pod kątem kluczowych wymagań metodologicznych: identycznego seeda danych wejściowych, wspólnego zestawu algorytmów (Bubble Sort, Quick Sort, Merge Sort oraz Insertion Sort), wspólnego katalogu wzorców danych wejściowych oraz jednakowego sposobu renderowania słupków jako prostych elementów DOM. Oznacza to, że interpretacja wyników powinna koncentrować się na zachowaniu mechanizmów renderowania i detekcji zmian frameworków, a nie na wpływie odmiennych bibliotek wykresów czy różnych sekwencji danych testowych.

\section{Wyniki pomiarów wydajności renderowania dla obu implementacji}
W tej części należy prezentować wyniki w układzie algorytm $\times$ framework $\times$ rozmiar danych $\times$ wzorzec wejścia, przy zachowaniu tego samego seeda dla każdej pary porównawczej. Szczególnie istotne jest rozróżnienie zachowania algorytmów kwadratowych (Bubble Sort, Insertion Sort) od algorytmów o średniej złożoności $O(n \log n)$ (Quick Sort, Merge Sort), ponieważ generują one odmienny rozkład porównań, zamian i aktualizacji wizualnych.

\section{Analiza zużycia zasobów i płynności animacji}
Analizując zużycie zasobów należy uwzględniać fakt, że obie aplikacje renderują tę samą strukturę słupków jako elementy DOM oraz wykorzystują identyczne kodowanie kolorystyczne stanów \textit{compare}, \textit{swap}, \textit{pivot} i \textit{sorted}. Dzięki temu obserwowane różnice w czasie \textit{Scripting}, \textit{Recalculate Style}, \textit{Layout}, \textit{Paint} czy stabilności FPS można interpretować jako efekt architektury Reacta i Angulara, a nie efekt użycia różnych bibliotek graficznych.

\section{Porównanie rozmiarów i złożoności pakietów}
Porównanie pakietów powinno być prowadzone z uwzględnieniem faktu, że wersja React nie korzysta już z biblioteki Recharts, a wersja Angular od początku opiera się na własnym komponencie wykresu. W rezultacie analiza rozmiaru bundli może być interpretowana bardziej bezpośrednio jako różnica między narzutem ekosystemów i bibliotek UI niż jako efekt obecności zewnętrznego silnika chartingowego tylko po jednej stronie porównania.

\section{Wyniki oceny łatwości rozwoju i produktywności programistycznej}
Ocena produktywności powinna uwzględniać, że obie aplikacje rozwiązują ten sam problem z użyciem możliwie prostych i lokalnych struktur stanu: w React poprzez autorski hook \texttt{useSorting}, a w Angularze poprzez klasę \texttt{SortingPlayer} oraz Sygnały. Taki układ pozwala porównywać nie tylko liczbę linii kodu czy złożoność komponentów, ale także czytelność przepływu danych i łatwość utrzymania synchronizacji wielu sorterów pracujących na tym samym seeded input.

\section{Porównawcza analiza zastosowania TypeScript w obu frameworkach}
W interpretacji tej części istotne jest, że oba projekty współdzielą ten sam model danych kroku wizualizacji oraz ten sam kontrakt generatorów kroków. Dzięki temu analiza TypeScriptu może koncentrować się na różnicach wynikających z modelu komponentowego i reaktywności frameworków, a nie z nieporównywalnych struktur domenowych.
