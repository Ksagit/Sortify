...
W sytuacjach, gdy logika aktualizacji stanu staje się złożona i zależy od wielu powiązanych ze sobą wartości, zaleca się stosowanie hooka \texttt{useReducer}. Implementuje on wzorzec zbliżony do architektury Redux, w którym zmiana stanu odbywa się poprzez dispatching actions (\textit{wysyłanie akcji}) do reducer (\textit{funkcji redukującej}). Podejście to sprzyja separacji logiki biznesowej od warstwy prezentacji i ułatwia testowanie kodu \cite{Learning_React_Third}.
