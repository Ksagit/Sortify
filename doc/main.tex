...
\begin{lstlisting}[language=tsx, caption={Przykład zarządzania stanem lokalnym za pomocą hooka \texttt{useState}.}, label={lst:react-usestate}]
...
\begin{lstlisting}[language=tsx, caption={Zastosowanie hooka \texttt{useReducer} w przypadku złożonej logiki aktualizacji stanu.}, label={lst:react-usereducer}]
...
\begin{lstlisting}[language=tsx, caption={Podstawowy przykład wykorzystania Context API do współdzielenia stanu.}, label={lst:react-context}]
...
\begin{lstlisting}[language=tsx, caption={Przykład optymalizacji renderowania komponentu z użyciem \texttt{useMemo} oraz \texttt{React.memo}.}, label={lst:react-memo}]
...
W aplikacjach wymagających wysokiej wydajności warto również stosować mechanizmy kolejkowania i opóźniania aktualizacji, dostępne poprzez \texttt{useTransition} lub \texttt{useDeferredValue}. Funkcje te wprowadzono w wersjach Reacta wspierających renderowanie współbieżne, co pozwala na priorytetyzowanie aktualizacji i zapewnia płynność interfejsu nawet w warunkach dużego obciążenia.
