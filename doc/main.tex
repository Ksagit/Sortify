...
function SortingVisualizer({ data }) {
  ...
  const handleUpdate = (newData) => {
    startTransition(() => {
      // Operacja o niższym priorytecie
  ...
  return (
    <div style={{ opacity: isPending ? 0.8 : 1 }}>
      {/* Wizualizacja na podstawie odroczonych danych */}
...
\begin{lstlisting}[language=tsx, caption={Zarządzanie stanem w Angularze z wykorzystaniem mechanizmu Sygnałów (\textit{Signals}).}, label={lst:angular-signals}]
@Component({
  selector: 'app-visualizer',
  standalone: true,
  template: `<div>Wartość: {{ count() }}</div>`
})
export class VisualizerComponent {
  // Definicja sygnału
  count = signal(0);
  
  // Wartość pochodna automatycznie aktualizowana
...
\begin{lstlisting}[language=tsx, caption={Szkielet asynchronicznego algorytmu sortowania (Bubble Sort) przygotowanego pod wizualizację.}, label={lst:algo-skeleton}]
  ...
  for (let i = 0; i < n; i++) {
    for (let j = 0; j < n - i - 1; j++) {
      if (array[j] > array[j + 1]) {
        ...
        // Aktualizacja stanu i wymuszenie przerwy dla renderera
        update([...array]);
