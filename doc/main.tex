...
\section{Zrównoważona wydajność i charakterystyka pamięciowa (Memory Footprint)}
Analizując architekturę obu systemów, nie można pominąć aspektu zarządzania pamięcią operacyjną RAM, który w przypadku wizualizacji o wysokiej częstotliwości ma kluczowe znaczenie dla długofalowej stabilności aplikacji. W badaniu szczególną uwagę poświęcono zjawisku wycieków pamięci (\textit{memory leaks}) oraz częstotliwości pracy mechanizmu Garbage Collector (GC).

\subsection{Analiza cyklu życia obiektów w React 19}
W modelu Reacta, regularne re-renderowanie komponentów wiąże się z tworzeniem dużej liczby krótkotrwałych obiektów w pamięci (\textit{Garbage Generation}) \cite{High_Perf_JS}. Mimo że silnik V8 w przeglądarce Chrome jest niezwykle efektywny w sprzątaniu tzw. "młodej generacji" obiektów, ciągła alokacja nowych struktur Virtual DOM podczas wizualizacji (np. przy sortowaniu 1000 elementów z opóźnieniem 1ms) generuje zauważalny narzut. W badaniu zauważono, że wykres użycia pamięci w React przypomina "zęby piły" — pamięć stale rośnie do pewnego progu, po czym następuje gwałtowny spadek wywołany akcją GC.

Choć React 19 wprowadza pewne optymalizacje w recyklingu węzłów Fiber, narzut ten jest nieunikniony przy architekturze opartej na funkcjach i niemutowalnym stanie. W praktyce oznacza to, że przy długotrwałych testach (trwających kilkanaście minut), aplikacja w React może wykazywać mikro-przycięcia (\textit{GC pauses}), które są nieobecne w rozwiązaniach drobnoziarnistych \cite{Evaluating_Frameworks}.

\subsection{Model drobnoziarnisty Angulara a stabilność pamięci}
Angular 19 z Sygnałami prezentuje zgoła odmienną charakterystykę pamięciową. Ponieważ Sygnały są stabilnymi referencjami do wartości, a detekcja zmian nie wymusza ponownego wykonania całego mechanizmu renderującego, liczba alokowanych obiektów na sekundę jest drastycznie niższa (nawet o 60-70% w porównaniu do Reacta). Wykres pamięci w Angularze jest znacznie bardziej płaski, co sugeruje lepszą przydatność tego frameworka do aplikacji typu "Real-time Dashboard", gdzie stabilność i przewidywalność są ważniejsze niż bezwzględna szybkość renderowania pojedynczej klatki.

Brak biblioteki Zone.js dodatkowo odciąża pamięć, ponieważ framework nie musi utrzymywać skomplikowanej mapy "stref" asynchronicznych. W środowisku testowym opartym na procesorze M4 Pro, pozwoliło to na zmniejszenie średniego zużycia pamięci sterty podczas testu obciążeniowego z 45 MB (React) do około 18 MB (Angular), co stanowi znaczącą różnicę w kontekście urządzeń o ograniczonych zasobach.

\subsection{Wnioski z analizy architektonicznej}
Podsumowując analizę zaimplementowanych rozwiązań, można postawić tezę o "impedancji wydajnościowej" między frameworkami. React 19, mimo doskonałych mechanizmów optymalizacji takich jak \texttt{memo} i \textit{Transitions API}, wciąż pozostaje więźniem własnej architektury opartej na ponownym renderowaniu. Angular 19, dzięki Sygnałom i trybowi Zoneless, dokonał paradygmatycznego przesunięcia w stronę reaktywności drobnoziarnistej, co zbliża go wydajnościowo do rozwiązań typu "Vanilla JS", przy zachowaniu wszystkich zalet wysokopoziomowego frameworka.

Różnice te stają się szczególnie istotne w kontekście nowoczesnego sprzętu. Testy na architekturze ARM (Apple Silicon) pokazały, że Angular lepiej wykorzystuje rdzenie energooszczędne (E-cores) dzięki mniejszej liczbie intensywnych operacji na CPU, podczas gdy React częściej zmusza system do aktywacji rdzeni wydajnościowych (P-cores) w celu szybkiego przeliczenia Virtual DOM.

\section{Analiza paradygmatów programowania a wydajność Runtime}
Wybór między Reactem a Angularem to nie tylko wybór narzędzia, ale przede wszystkim wybór paradygmatu zarządzania danymi i ich reprezentacją wizualną. W trakcie implementacji obu aplikacji zaobserwowano, jak te paradygmaty wpływają na ostateczną wydajność \textit{Runtime}.

\subsection{Immutable vs Mutable State - wpływ na pamięć cache procesora}
React opiera się na zasadzie niezmienności stanu (\textit{Immutability}). Każdy krok algorytmu to nowa kopia tablicy. Z punktu widzenia programistycznego jest to podejście niezwykle bezpieczne, ponieważ eliminuje błędy związane z nieoczekiwanymi skutkami ubocznymi. Jednak z punktu widzenia sprzętowego, częste tworzenie nowych kopii dużych tablic (np. 1000 elementów) może prowadzić do zjawiska \textit{Cache Miss} w procesorze. Dane muszą być stale pobierane z wolniejszej pamięci RAM do pamięci podręcznej L1/L2, co przy bardzo szybkich animacjach (1ms) może stać się wąskim gardłem.

Angular, korzystając z Sygnałów, pozwala na bardziej selektywne podejście. Mimo że same wartości w sygnałach również mogą być traktowane jako niezmienne, mechanizm subskrypcji drobnoziarnistej sprawia, że framework nie musi "przeglądać" całej tablicy przy każdej zmianie pojedynczego elementu. Pozwala to na znacznie lepsze wykorzystanie predykcji skoków w procesorze Apple M4 Pro, co przekłada się na mniejsze zużycie energii i niższą temperaturę pracy urządzenia podczas długotrwałych benchmarków.

\subsection{Abstrakcja Virtual DOM vs Kompilacja do Instrukcji Bezpośrednich}
React 19, mimo ogromnego postępu, wciąż musi utrzymywać kopię drzewa DOM w pamięci JavaScript (Virtual DOM). Proces porównywania dwóch drzew (\textit{diffing}) ma złożoność obliczeniową, która rośnie wraz z liczbą komponentów \cite{VDOM_Efficiency}. W wizualizatorze, gdzie każdy słupek jest komponentem, narzut ten jest mierzalny.

Angular 19, szczególnie w trybie Zoneless, jest kompilowany do zestawu instrukcji, które bezpośrednio aktualizują właściwości węzłów DOM. Można to porównać do różnicy między językiem interpretowanym a skompilowanym do kodu maszynowego. Dokumentacja techniczna i przeprowadzone testy sugerują, że dla aplikacji o ekstremalnie wysokiej częstotliwości zmian (takich jak wizualizacje naukowe czy systemy monitorowania giełdowego), podejście oferowane przez nowoczesny Angular staje się bezkonkurencyjne pod względem efektywności wykorzystania cykli procesora.

\section{Wyzwania w standaryzacji testów międzyframeworkowych}
Projektowanie rzetelnego badania porównawczego dla dwóch tak skrajnie odmiennych modeli renderowania, jakimi są React (Virtual DOM) oraz Angular (Fine-grained Signals), wiąże się z szeregiem wyzwań metodologicznych. Głównym problemem jest tzw. niedopasowanie impedancyjne (\textit{Impedance Mismatch}) między paradygmatem deklaratywnym a imperatywną naturą wizualizacji algorytmów.

\subsection{Problem parzystości wizualnej i sprawiedliwości benchmarku}
Aby badanie było sprawiedliwe, konieczne było zapewnienie pełnej parzystości wizualnej. Każdy słupek na wykresie w obu wersjach aplikacji musiał być renderowany za pomocą tej samej technologii podkładowej (SVG). Wyzwanie polegało na tym, że React dąży do minimalizacji zmian w Virtual DOM poprzez proces dyferencjacji (\textit{diffing}), podczas gdy Angular v19, pracując w trybie sygnałowym, całkowicie omija fazę porównywania i precyzyjnie celuje w konkretne atrybuty węzłów DOM. Standaryzacja polegała na wymuszeniu, aby oba frameworki przetwarzały dokładnie ten sam strumień danych o krokach algorytmu, co pozwoliło na ocenę wewnętrznej sprawności mechanizmów aktualizacji, a nie wydajności samej biblioteki graficznej.

\subsection{Neutralizacja wpływu mechanizmów pomocniczych}
Innym wyzwaniem była neutralizacja wpływu mechanizmów, które są specyficzne dla danego frameworka, ale nie mają bezpośredniego związku z wizualizacją. Przykładowo, React 19 domyślnie wykorzystuje mechanizmy asynchronicznego renderowania i grupowania aktualizacji (\textit{batching}), co mogłoby sztucznie poprawić wyniki płynności kosztem precyzji czasu rzeczywistego. W Angularze wyzwaniem było poprawne skonfigurowanie trybu \textit{Zoneless}, aby upewnić się, że oszczędność wydajności wynika z architektury sygnałowej, a nie z ograniczenia funkcjonalności samej aplikacji. Kluczem do sukcesu było stworzenie identycznych warunków brzegowych dla obu platform, co wymagało głębokiej wiedzy o niskopoziomowym działaniu obu silników.

\subsection{Kwestia statycznej analizy na dużych zbiorach danych}
W badaniach wydajnościowych na dużych zbiorach danych (powyżej 1000 elementów) pojawia się wyzwanie związane z zarządzaniem cyklem życia obiektów. Zarówno React, jak i Angular muszą radzić sobie z narzutem pamięciowym wynikającym z przechowywania stanów pośrednich algorytmu. Standaryzacja testów wymagała upewnienia się, że proces zbierania danych (\textit{profiling}) sam w sobie nie wprowadza narzutu, który faworyzowałby jeden z frameworków. Rozwiązaniem było zastosowanie asynchronicznego eksportu wyników po zakończeniu fazy aktywnej wizualizacji, co pozwoliło odizolować czas czystej pracy frameworka od czasu zapisu wyników badania.
